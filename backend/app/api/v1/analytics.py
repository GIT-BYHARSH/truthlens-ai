...
@router.get("/trends")
async def analytics_trends(db: AsyncSession = Depends(get_db)) -> dict:
    ...
    rows = await db.execute(
        select(
            ...
            cast(func.avg(Verification.credibility_score), Float).label("avg_credibility"),
            cast(func.avg(Verification.confidence_score), Float).label("avg_confidence"),
        )
        .group_by("day")
        .order_by("day")
    )
